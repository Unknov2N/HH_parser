...
CHARS_TO_DELETE = r'\/:*?"<>|"'  # for naming identical files
...
with os.scandir(PATH) as dirs:
    # open file named query, contains vacancy`s ID
    for dirname in (dir.name for dir in dirs if dir.is_dir()):
        dirpath = PATH + dirname
        if not os.path.exists(dirpath):
            # создаём отдельную папку для каждого запроса
            os.mkdir(dirpath)
        # перебираем файлы-ответы
        with open(dirpath + '/vacancy_ids.dat') as vacancy_ids:
            delete_all_files_in_dir(dirpath + '/vacancies_description')
            time.sleep(1)  # задержка между text={}
            error_index = itertools.count()
            uniq_names = dict()
            uniq_key_skills = dict()

            get_vacancies_data_log = open(PATH + dirname + '/logs.log', 'a', encoding="UTF-8")
            for indx, id in enumerate(vacancy_ids.readlines()):
                ...
                file_output_name = ''.join([c for c in data['name'] if c not in CHARS_TO_DELETE])
                ...
                if not file_output_name in uniq_names:  # os.path.exists(filepath + '/' + file_output_name + '.json'):
                    uniq_names[file_output_name] = dict()
                    uniq_names[file_output_name]['count'] = 1
                    uniq_names[file_output_name]['key_skills'] = dict()
                    for skill in job_info['key_skills']:
                        uniq_names[file_output_name]['key_skills'][skill] = 1
                    ...
                    full_file_output_name = dirpath + '/vacancies_description/' + file_output_name + '.json'
                    with open(full_file_output_name, 'w', encoding="UTF-8") as file_output:
                        file_output.write(json.dumps(job_info, ensure_ascii=False))
                        print(indx, id[:-1], file_output_name)
                else:
                    full_file_output_name = dirpath + '/vacancies_description/' + file_output_name + ' (' \
                                            + str(uniq_names[file_output_name]['count']) + ').json'
                    #get_vacancies_data_log.write(f'rename file, PATH: {full_file_output_name}\n')
                    with open(full_file_output_name, 'w', encoding="UTF-8") as file_output:
                        file_output.write(json.dumps(job_info, ensure_ascii=False))
                        print(f'{indx} {id[:-1]} '
                              f'{file_output_name} ({uniq_names[file_output_name]["count"]})')
                    uniq_names[file_output_name]['count'] += 1
                    for skill in job_info['key_skills']:
                        if skill in uniq_names[file_output_name]['key_skills']:
                            uniq_names[file_output_name]['key_skills'][skill] += 1
                        else:
                            uniq_names[file_output_name]['key_skills'][skill] = 1
...
# добавить в название ключевые навыки и их количество. сортируем сначала по количеству, потом по алфавиту (де-факто наоборот) (сделать функцию добавления количества и сортировки)
            ...
            with open(dirpath + '/uniq_names.json', 'w', encoding='UTF-8') as f:
            ...
            with open(dirpath + '/uniq_key_skills.json', 'w', encoding='UTF-8') as f:
